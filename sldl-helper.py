...
def download_song(title, artist, uri, output_path, log_file):
...
def main():
    ...
    if state == "Downloaded":
        ...
        # TODO: append some info to the log file
    # if sldl did not find the song, download it from youtube
    elif state == "Failed":
        ytdlp_output = download_song(title, artist, uri, OUTPUT_PATH, LOG_FILE_PATH)
        
        # if the download was successfull extract the filepath from the yt-dlp output and build the correct index entry
        if ytdlp_output.count(f"[download] Finished downloading playlist: {title} {artist}") == 1:
