...
def dump_json(contents, filename):
    with open("assets/" + filename, "w", encoding="utf-8") as file:
        json.dump(contents, file)

def get_all_playlists(json_filename=None):
    ...
    if json_filename is not None:
        dump_json(all_playlists, json_filename)

    return all_playlists

def get_all_playlist_tracks(playlist_id, json_filename=None):
    all_tracks = []
    offset = 0
    ...
    if json_filename is not None:
        dump_json(all_tracks, json_filename)
